...
class ASTP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.global_context_att:
            context_mean = torch.mean(x, dim=-1, keepdim=True)
            context_std = torch.sqrt(torch.var(x, dim=-1, keepdim=True) + 1e-7)

            # Dùng broadcasting để nhân lên cùng kích thước với x
            context_mean = context_mean * torch.ones_like(x)
            context_std = context_std * torch.ones_like(x)
