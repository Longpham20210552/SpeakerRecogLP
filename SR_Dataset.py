...
class TruncatedInputfromMFB_NotRandom(object):
    def __init__(self, input_per_file=1):
        super(TruncatedInputfromMFB_NotRandom, self).__init__()
...
class TruncatedInputfromMFB(object):
    """
    input size : (n_frames, dim=60)
    output size : (1, n_win=200, dim=60) => one context window is chosen randomly
    """
    def __init__(self, input_per_file=1):
...
class DvectorDataset_2(data.Dataset):
    def __init__(self, DB, loader, spk_to_idx, transform=None, *arg, **kw):
        """
        Dataset cho các dvector file.

        Args:
            DB: DataFrame chứa thông tin file và nhãn.
            loader: Hàm để tải dữ liệu từ file.
            spk_to_idx: Từ điển ánh xạ từ nhãn sang chỉ số.
            transform: Các hàm chuyển đổi dữ liệu đầu vào (nếu cần).
        """
        self.DB = DB
        self.len = len(DB)
        self.transform = transform
        ...
        # Đảm bảo nhãn 'unknown' tồn tại
        if 'unknown' not in self.spk_to_idx:
            self.spk_to_idx['unknown'] = len(self.spk_to_idx)

    def __getitem__(self, index):
        """
        Lấy dữ liệu tại một chỉ số nhất định.

        Args:
            index: Vị trí của dữ liệu cần lấy.

        Returns:
            feature: Đặc trưng của tín hiệu.
            label: Nhãn được chuyển thành chỉ số.
        """
        feat_path = self.DB['filename'][index]
        feature, label = self.loader(feat_path)
        
        # Kiểm tra nếu nhãn không tồn tại trong từ điển, gán nhãn 'unknown'
...
def main():
    ...
    batch_size = 128
    Dvector_train_dataset = DvectorDataset(DB=train_DB, loader=file_loader, transform=transform, spk_to_idx=spk_to_idx)
